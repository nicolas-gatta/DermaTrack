...
class H5ImagesDataset(Dataset):
    ...
    def __getitem__(self, index):
        try:
            self._init_h5_file()
            lr = torch.from_numpy(self.lr_images[f"image_{index + 1}"][:]).permute(2, 0, 1).float() / 255
            hr = torch.from_numpy(self.hr_images[f"image_{index + 1}"][:]).permute(2, 0, 1).float() / 255
            return lr, hr
    ...
    def get_raw_data(self, index):
        try:
            lr = self.lr_images[f"image_{index + 1}"][:]
            hr = self.hr_images[f"image_{index + 1}"][:]
            return lr, hr
