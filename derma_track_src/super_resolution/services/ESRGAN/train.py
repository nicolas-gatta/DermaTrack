...
def train_model(model_name: str, train_file: str, valid_file: str, eval_file: str, output_path: str, 
    ...
    generator = ESRGANGenerator(up_scale = scale).to(device)
    
    pretrained_model_name = f"RRDB_PSNR_x{scale}_{mode}.pth"
    ...
    discriminator = ESRGANDiscriminator(crop_size = crop_size).to(device)
    ...
    torch.save({"architecture": "ESRGAN", "scale": scale, "color_mode": mode, "invert_color_mode": invert_mode, 
...
def pretrained_model(model: ESRGANGenerator, train_loader: DataLoader, val_loader: DataLoader, eval_file: str, 
    ...
    JsonManager.copy_model_data_and_update(source_model_name = SRGAN_model_name, destination_model_name = model_name, updated_fields = {ModelField.ARCHITECTURE: "RRDBNet"})
    ...
    torch.save({"architecture": "RRDBNet", "scale": scale, "color_mode": mode, "invert_color_mode": invert_mode, 
